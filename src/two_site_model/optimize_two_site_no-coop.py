...
def main():
    ...
    ax.plot(N, np.max(f, axis=1), label=r"Maximum IFN$\beta$", linewidth=3)
    ax.fill_between(N, np.min(f, axis=1), np.max(f, axis=1), alpha=0.2, label = "Contribution of IRF")
    ax.plot(N, np.min(f, axis=1), label=r"Minimum IFN$\beta$", linewidth=3)
    ...
    fig, ax = plt.subplots()
    ax.set_prop_cycle("color", plt.cm.viridis(np.linspace(0, 1, 5)))
    ax.plot(I, np.max(f, axis=0), label=r"Maximum IFN$\beta$", linewidth=3)
    ax.fill_between(I, np.min(f, axis=0), np.max(f, axis=0), alpha=0.2, label = "Contribution of NF$\kappa$B")
    ax.plot(I, np.min(f, axis=0), label=r"Minimum IFN$\beta$", linewidth=3)
    ax.set_xlabel(r"$IRF$")
    ax.set_ylabel(r"IFN$\beta$")
    # ax.set_title("Model %s" % model)
