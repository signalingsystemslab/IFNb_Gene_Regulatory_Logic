...
def minimize_objective(pars, N, I, beta, h_pars, bounds, c_par=None):
    return opt.minimize(three_site_objective, pars, args=(N, I, beta, c_par, h_pars), method='trust-constr', bounds=bounds)
...
def main():
    ...
    pars_dir = "three_site_param_scan_hill/results"
    ...
    for row in h_vals:
        ...
        initial_pars = pd.read_csv("%s/%s_all_best_20_pars_h_%s.csv" % (pars_dir, model, h_vals_str))
