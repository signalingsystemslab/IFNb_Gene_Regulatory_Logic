...
def plot_parameters_one_plot(pars_0, pars_1, pars_2, pars_3, name, figures_dir, hi2=[], hi1=[], hn=[], ordered = False):
    ...
    if len(hi2) > 0:
        ...
        if len(hn) > 0:
            ...
            # Model names (Hills given, no NFkB Hill)
            df_all_t_pars["Model"] = r"$h_{I_1}$=" + df_all_t_pars["H_{I_1}"] + r", $h_{I_2}$=" + df_all_t_pars["H_{I_2}"]
...
def get_contribution_data(num_t_pars=5, num_k_pars=4, results_dir="parameter_scan_dist_syn/three_site_contrib/", num_threads=40):
    h_pars = "3_1_1"
    ...
    np.savetxt("%s/three_site_state_names.txt" % (results_dir), state_names, fmt="%s")
    ...
    inputs = [(tpars, kpars, n, i, None, hpars) for tpars, kpars, hpars in zip(best_tpars, best_kpars, best_hpars) for n, i, p50 in zip(filtered_training_data["NFkB"].values, filtered_training_data["IRF"].values, filtered_training_data["p50"].values)]
...
def make_contribution_plots():
    ...
    os.makedirs(figures_dir, exist_ok=True)
    training_data = pd.read_csv("../data/training_data.csv")
    results_dir = "parameter_scan_dist_syn/three_site_contrib/"
    os.makedirs(results_dir, exist_ok=True)
    model = "three_site_dist_syn"
    # best_20_pars_df = pd.read_csv("%s/%s_all_best_20_pars_h_%s.csv" % (best_fit_dir, model, h))
    cmap = heatmap_cmap

    get_contribution_data(num_t_pars=5, num_k_pars=3)
    ...
    old_state_names = np.loadtxt("%s/three_site_state_names.txt" % results_dir, dtype=str, delimiter="\0")
    state_name_dict = get_renaming_dict(old_state_names)
    ...
    contrib_df["genotype"] = pd.Categorical(contrib_df["genotype"], categories=genotype_levels, ordered=True).remove_unused_categories()
...
def make_state_probabilities_plots():
    ...
    names_dir = "parameter_scan_dist_syn/three_site_contrib"
    num_t_pars = 5
    num_k_pars = 3
    ...
    # print(state_probs_df["state"].unique())
    ...
    # ['Unbound' '$IRF_1$' '$IRF_2$' '$NF\\kappa B$' '$IRF_1\\cdot IRF_2$'
#  '$IRF_1\\cdot NF\\kappa B$' '$IRF_2\\cdot NF\\kappa B$'
#  '$IRF_1\\cdot IRF_2\\cdot NF\\kappa B$']
    # self.t[1] = self.parsT[0] # t1 - IRF2
    #     self.t[2] = self.parsT[0] # t1 - IRF1
    #     self.t[3] = self.parsT[1] # t3 - NFkB
    #     self.t[4] = self.parsT[2] # t4 - IRF1&IRF2
    #     self.t[5] = self.parsT[4] # t6 - IRF2&NFkB
    #     self.t[6] = self.parsT[3] # t5 - IRF1&NFkB
    #     self.t[7] = 1
...
def plot_max_resid(df, figures_dir, name=""):
    ...
    with sns.plotting_context("paper", rc=plot_rc_pars):
        ...
        df["Cooperativity"] = df["Cooperativity"].replace({"c": "+"})
...
def make_supplemental_plots():
    ...
    c_scan_models = ["h_%d_%d_%d_c_scan" % (h1.ravel()[i], h2.ravel()[i], h3.ravel()[i]) for i in range(len(h1.ravel()))]
    ...
    for m in models:
        if "3_1_1" in m:
            ...
            predictions = np.loadtxt("%s/%s_best_fits_ifnb_predicted.csv" % ("%s/results%s" % (force_t_dir, coop), model), delimiter=",")
        else:
            fname = "%s/%s_best_fits_ifnb_predicted.csv" % ("%s/results_%s" % (force_t_dir, m), model)
    ...
    # Plot predictions of all_models with only one Hill coefficient
    print("Plotting predictions for all hill combinations on one plot", flush=True)
    predictions_1_1_1 = np.loadtxt("%s/%s_best_fits_ifnb_predicted.csv" % ("%s/results_h_1_1_1/" % scan_dir, model), delimiter=",")
    ...
    best_20_pars_1_1_1 = pd.read_csv("%s/%s_best_fits_pars.csv" % ("%s/results_h_1_1_1/" % scan_dir, model))
    best_20_pars_1_3_1 = pd.read_csv("%s/%s_best_fits_pars.csv" % ("%s/results_h_1_3_1/" % scan_dir, model))
