...
def get_path(force_k=False,c=False,h1=3,h2=1):
    path = "parameter_scan"
    if force_k:
        path += "_force_k/results"
    else:
        path += "/results"
    h_val = "%d_%d_1" % (h1, h2)
    if h_val != "3_1_1":
    ...
    if c:
        path += "_c_scan"
    path += "/p50_diff_binding_best_fits_pars.csv"
    return path
...
def plot_rmsd_boxplot(rmsd_df_input, name, figures_dir):
    ...
    with sns.plotting_context("talk", rc={"lines.markersize": 7}):
        ...
        table_data = rmsd_df[[r"$h_1$", r"$h_2$", "c", r"$k_1=k_2$"]].drop_duplicates().values.tolist()
        ...
        table = plt.table(cellText=table_data, cellLoc='center', loc='bottom', rowLabels=[r"$h_1$", r"$h_2$", "c",r"$k_1=k_2$"], bbox=[0, -0.35, 1, 0.3])
...
def main():
    ...
    h1s = [3,1]
    h2s = [1,3]
    # conditions = ["%d_%d_%s_%s" % (h1, h2, c, f) for h1 in h1s for h2 in h2s for c in cs for f in force_ks]
    # print(conditions)
    
    args = [(f, c, h1, h2) for h1 in h1s for h2 in h2s for c in cs for f in force_ks]
    ...
    rmsd = np.array(rmsd)
    print(rmsd.shape)
    df = pd.DataFrame({r"$k_1=k_2$": [arg[0] for arg in args], 
                       "c": [arg[1] for arg in args],
                        r"$h_1$": [arg[2] for arg in args],
                        r"$h_2$": [arg[3] for arg in args]})
    df["Condition"] = df[r"$h_1$"].astype(str) + "_" + df[r"$h_2$"].astype(str) + "_" + df["c"].astype(str) + "_" + df[r"$k_1=k_2$"].astype(str)
    df["Condition"] = pd.Categorical(df["Condition"], conditions)
    for i in range(rmsd.shape[1]):
        df["rmsd_%d" % i] = rmsd[:,i]
    # df = df.melt(id_vars=["force_k", "c", "h1", "h2"], var_name="par_set", value_name="rmsd")
    df = df.melt(id_vars=[r"$k_1=k_2$", "c", r"$h_1$", r"$h_2$","Condition"], var_name="par_set", value_name="rmsd")
    print(df)
