...
from parameter_scan_p50_model import plot_state_probabilities, plot_parameter_distributions, get_N_I_P
...
def calc_state_prob(k_pars, N, I, P, h_pars, num_t):
...
def p50_objective(pars, *args):
    ...
    N, I, P, beta, h_pars, c_par, which_pars, other_pars = args

    if which_pars == "All":
        if c_par is None:
            if len(pars) != len(other_pars):
                raise ValueError("Number of parameters (%d) does not match number of t and k parameters (%d)" % (len(pars), len(other_pars)))
        else:
            if len(pars) != len(other_pars) + 1:
                raise ValueError("Number of parameters (%d + c) does not match number of t and k parameters (%d)" % (len(pars) -1, len(other_pars)))
    elif which_pars == "alltk":
        if len(pars) != len(other_pars):
            raise ValueError("Number of parameters (%d) does not match number of t and k parameters (%d)" % (len(pars), len(other_pars)))
...
def minimize_objective(pars, N, I, P, beta, h_pars, c_par, which_pars, other_pars, bounds):
    return opt.minimize(p50_objective, pars, args=(N, I, P, beta, h_pars, c_par, which_pars, other_pars), method="Nelder-Mead", bounds=bounds)

def optimize_model(N, I, P, beta, initial_pars, h, c, opt_pars, num_threads=40):
    '''
    ...
    if opt_pars == "All":
        ...
        if c is not None:
            bnds.append((min_c, max_c))
            pars = np.concatenate((initial_pars, np.repeat(c, len(initial_pars)).reshape(-1,1)), axis=1)
    elif opt_pars == "alltk":
        bnds = [(0,1) for i in range(5)]
        bnds += [(10**min_k_order, 10**max_k_order) for i in range(4)]
        pars = initial_pars
    ...
    with Pool(num_threads) as p:
        results = p.starmap(minimize_objective, [(pars[i], N, I, P, beta, h, c, opt_pars, initial_pars[i], bnds) for i in range(len(pars))])
...
def main():
    ...
    parser.add_argument("-c","--optimize_c", action="store_true")
    ...
    if c is not None:
        figures_dir = figures_dir[:-1] + "_c_%d/" % c
        results_dir = results_dir[:-1] + "_c_%d/" % c
        if opt_c:
            figures_dir = figures_dir[:-1] + "_opt_c/"
            results_dir = results_dir[:-1] + "_opt_c/"
    ...
    if c is None or not opt_c:
        result_par_names = initial_pars_names + ["h1", "h2"]
    else:
        result_par_names = initial_pars_names + ["c", "h1", "h2"]

    # Calculate rmsd for initial parameters
    beta_stacked = np.stack([beta for _ in range(len(initial_pars))], axis=0)
    with Pool(num_threads) as p:
        results = p.starmap(calculate_ifnb, [(pars, training_data, [h1, h2], c) for pars in initial_pars])
    ...
    if args.optimize:
        ...
        if opt_c:
            ...
            par_names="alltk"
        final_pars, rmsd = optimize_model(N, I, P, beta, initial_pars, [h1, h2], c, par_names, num_threads=num_threads)
    ...
    if not args.optimize_kp:
        ...
        if args.state_probabilities:
            ...
            all_k_pars = final_pars[:, num_t_pars:-num_h_pars]
            ...
            for stimulus, genotype in zip(stimuli, genotypes):
                        ...
                        with Pool(num_threads) as p:
                            results = p.starmap(calc_state_prob, [(tuple(all_k_pars[i]), nfkb, irf, p50, [h1, h2], num_t_pars) for i in range(len(all_k_pars))])
