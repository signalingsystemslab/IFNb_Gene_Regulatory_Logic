...
def calculate_grid(training_data, h1=3, h2=1, t_bounds=(0,1), k_bounds=(10**-3,10**3), seed=0, num_samples=10**6, num_threads=60, num_t_pars=5, num_k_pars=4, num_h_pars=2, c_par=False):
    ...
    if c_par:
        with Pool(num_threads) as p:
            results = p.starmap(get_f, [(grid[i,0:5], grid[i,5:-1], N[j], I[j], P[j], grid[i,-1], [h1, h2], False) for i in range(len(grid)) for j in range(len(N))])
    else:
        with Pool(num_threads) as p:
            results = p.starmap(get_f, [(grid[i,0:5], grid[i,5:], N[j], I[j], P[j], None, [h1, h2], False) for i in range(len(grid)) for j in range(len(N))])
...
def main():
    ...
    if args.param_scan:
        ...
        final_pars, ifnb_predicted, rmsd = calculate_grid(training_data, seed=0, num_threads=num_threads, h1=h1, h2=h2, c_par=c_par)
        print("Finished parameter scan.", flush=True)

        if c_par:
            result_par_names = ["t1", "t2", "t3", "t4", "t5", "k1", "k2", "kn", "kp", "c"]
        else:
            result_par_names = ["t1", "t2", "t3", "t4", "t5", "k1", "k2", "kn", "kp"]
