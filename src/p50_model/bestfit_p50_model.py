...
def plot_all_models(plims=[0,2]):
    ...
    fig6_data = pd.read_csv("../data/p50_testing_data.csv")
    # fig6_data["p50"] = [plims[0], model.iloc[50,model.columns.get_loc("p50")], plims[1]]
    ...
    for model_name in ["B1", "B2", "B3", "B4"]:
        ...
        model["ifnb"] = model["ifnb"] / model.iloc[norm_p50,model.columns.get_loc("ifnb")] * fig6_data.iloc[norm_p50, fig6_data.columns.get_loc("IFNb")]
        plt.plot(model["p50"], model["ifnb"], label=model_name, linewidth=5)
    plt.scatter(fig6_data["p50"], fig6_data["IFNb"], s=75, label="Data", zorder =2, color="black")
    plt.xlabel("p50")
    plt.ylabel(r"IFN$\beta$")
    plt.ylim([0,1])
    plt.title("Model predictions for all models")
    fig.legend(bbox_to_anchor=(1.1,0.5))
    plt.savefig(dir+"all_models%d-%f.svg" % (plims[0], plims[1]))
...
# #  t parameters
b1_t_pars = [0, 0.21992227, 0.75186586, 0.10526895, 0.9766144,  1., 1., 1.]
b2_t_pars = [0, 0.01995081, 0.50188124, 0.13282681, 1., 1., 1., 1.]
x1=np.arange(8)-0.4
x2=np.arange(8)

# fig = plt.figure()
# ax1 = plt.subplot(111)
# plt.barh(x1, b1_t_pars, label="B2", height=0.4, align="edge", color="#348ABD")
# plt.barh(x2, b2_t_pars, label="B4", height=0.4, align="edge", color="#777777")
# fig.legend(bbox_to_anchor=(1.05,0.5))
# plt.yticks(np.arange(8), ["No bound",r"IRF", r"$IRF_G$", r"NF$\kappa$B", r"IRF $IRF_G$", r"IRF NF$\kappa$B", r"$IRF_G$ NF$\kappa$B",r"IRF $IRF_G$ NF$\kappa$B"])
# plt.xlabel("Transcription capability (t)")
# ax1.invert_yaxis()
# plt.savefig("./figures/optimized/p50-p50_best_fit_tpars.svg")

#  state vectors
lps_wt = Modelp50(np.insert(b1_t_pars[1:-1], 0, 2), "B2")
lps_wt.calculateState(1, 0.1,1)
lps_wt_state = lps_wt.state
# print(lps_wt_state)
lps_ko = Modelp50(np.insert(b1_t_pars[1:-1], 0, 2), "B2")
lps_ko.calculateState(0.5,0.1,0)
lps_ko_state = lps_ko.state

pic_wt = Modelp50(np.insert(b1_t_pars[1:-1], 0, 2), "B2")
pic_wt.calculateState(0.5, 0.75, 1)
pic_wt_state = pic_wt.state
pic_ko = Modelp50(np.insert(b1_t_pars[1:-1], 0, 2), "B2")
pic_ko.calculateState(0.25, 0.75, 0)
pic_ko_state = pic_ko.state

fig = plt.figure()
ax1 = plt.subplot(121)
plt.barh(np.arange(12), lps_wt_state, label="WT", height=0.4, align="edge", color="black")
plt.barh(np.arange(12)-0.4, lps_ko_state, label="KO", height=0.4, align="edge", color="#EF4A5F")
ax1.invert_yaxis()
plt.yticks(np.arange(12), ["No bound",r"IRF", r"$IRF_G$", r"IRF p50", r"NF$\kappa$B", r"NF$\kappa$B p50",
                          "p50", r"IRF $IRF_G$", r"IRF NF$\kappa$B", r"$IRF_G$ NF$\kappa$B", r"IRF NF$\kappa$B p50",
                          r"IRF $IRF_G$ NF$\kappa$B"])

ax2 = plt.subplot(122, sharey=ax1)
plt.barh(np.arange(12), pic_wt_state, label="WT", height=0.4, align="edge", color="black")
plt.barh(np.arange(12)-0.4, pic_ko_state, label="KO", height=0.4, align="edge", color="#EF4A5F")
fig.legend(bbox_to_anchor=(1.05,0.5))
...
plt.savefig("./figures/optimized/p50-p50_example_state_vectors.svg")
