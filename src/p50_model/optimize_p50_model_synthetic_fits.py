...
def p50_objective(pars, *args):
    ...
    # f-values are normalized to pIC WT
...
def main():
    ...
    # print("Saving p50 model with grid optimization at %s" % time.ctime())
    ...
    fig, ax = plt.subplots(figsize=(8,6), dpi=300)
    for i in range(num_datasets):
        dots = ax.scatter(np.arange(num_pts), residuals[i,:], c="k", s=50, alpha=0.5)
        dots = jitter_dots(dots)
    ...
    N = training_data.loc[(training_data["Dataset"] == datasets[0]) & (training_data["Genotype"] == "WT")]["NFkB"].values
    P = 1.0
    num_points_WT = len(N)
    f_values_WT = np.zeros((num_datasets, num_points_WT))
    max_f = np.zeros(num_datasets)
    for i in range(num_datasets):
        N = training_data.loc[(training_data["Dataset"] == datasets[i]) & (training_data["Genotype"] == "WT")]["NFkB"].values
        I = training_data.loc[(training_data["Dataset"] == datasets[i]) & (training_data["Genotype"] == "WT")]["IRF"].values
        f_values_WT[i,:] = [get_f(pars_all[i,:], 1, 1, N[j], I[j], P, model, True) for j in range(num_points_WT)]
    ...
    fig, ax = plt.subplots(figsize=(8,6), dpi=300)
    for i in range(num_datasets):
        p = ax.scatter(N, I, c=f_values_WT[i,:], cmap=cmap, s=25, alpha=0.5, vmin=0, vmax=1)
        p = jitter_dots(p, y_jitter=True, jitter=0.05)
    ax.set_xlabel(r"$NF\kappa B$")
    ax.set_ylabel(r"$IRF$")
    ax.set_title("Model %s f-values for WT dataset" % model)
    # plt.colorbar(label="f-value")
    plt.colorbar(p, label="f-value")
    ...
    for i in range(10):
        ...
        for j in range(len(N)):
            for k in range(len(I)):
                f_values[j,k] = get_f(t_pars, K, C, N[j], I[k], P[k], model, True)
