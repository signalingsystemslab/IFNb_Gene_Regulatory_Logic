...
def main():
    ...
    if best_model == "B1":
        best_pars = best_results[0:6]
    elif best_model == "B2":
        best_pars = np.hstack([best_results[6], best_results[0:6]])
    elif best_model == "B3":
        best_pars = np.hstack([best_results[7], best_results[0:6]])
    elif best_model == "B4":
        best_pars = np.hstack([best_results[6:8], best_results[0:6]])
    f_list_best = [explore_modelp50(best_pars, N[i], I[i], P[i], best_model) for i in range(num_pts)]
    print("Predicted IFNb: ", f_list_best)
