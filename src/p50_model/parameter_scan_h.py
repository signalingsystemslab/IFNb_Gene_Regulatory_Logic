...
def calculate_grid(training_data, h_bounds=(1,4), t_bounds=(0,1), k_bounds=(10**-3,10**3), seed=0, num_samples=10**6, num_threads=60, num_t_pars=5, num_k_pars=4, num_h_pars=2, c_par=False):
    ...
    l_bounds = np.concatenate([np.zeros(num_t_pars)+min_t, np.ones(num_k_pars)*min_k_order, np.ones(num_h_pars)*h_bounds[0]])
    u_bounds = np.concatenate([np.zeros(num_t_pars)+max_t, np.ones(num_k_pars)*max_k_order, np.ones(num_h_pars)*h_bounds[1]])

    print("Calculating grid with %d samples using Latin Hypercube sampling" % num_samples, flush=True)
    print("Number of t_pars: %d\nNumber of h_pars: %d\nNumber of k_pars: %d" % (num_t_pars, num_h_pars, num_k_pars), flush=True)
    if not c_par:
        sampler=qmc.LatinHypercube(d=num_t_pars+num_k_pars+num_h_pars, seed=seed)
        ...
        kgrid = grid[:,num_t_pars:num_t_pars+num_k_pars]
        kgrid = 10**kgrid
        grid[:,num_t_pars:num_t_pars+num_k_pars] = kgrid
    else:
        l_bounds = np.append(l_bounds, min_c_order)
        u_bounds = np.append(u_bounds, max_c_order)
        sampler=qmc.LatinHypercube(d=num_t_pars+num_k_pars+num_h_pars+1, seed=seed)
        grid = sampler.random(n=num_samples)
        grid = qmc.scale(grid, l_bounds, u_bounds) # rows are parameter sets
        # convert k parameters to log space
        kgrid = grid[:,num_t_pars:num_t_pars+num_k_pars]
    ...
    if c_par:
        with Pool(num_threads) as p:
            results = p.starmap(get_f, [(grid[i,0:num_t_pars], # t_pars
                                         grid[i,num_t_pars:num_t_pars+num_k_pars], # k_pars
                                         N[j], I[j], P[j], # N, I, P
                                         grid[i,-1], # c_par
                                         grid[i,num_t_pars+num_k_pars:-1], # h_pars
                                         False) for i in range(len(grid)) for j in range(len(N))])
    else:
        with Pool(num_threads) as p:
            results = p.starmap(get_f, [(grid[i,0:num_t_pars], # t_pars
                                            grid[i,num_t_pars:num_t_pars+num_k_pars], # k_pars
                                            N[j], I[j], P[j], # N, I, P
                                            None, # c_par
                                            grid[i,num_t_pars+num_k_pars:], # h_pars
                                            False) for i in range(len(grid)) for j in range(len(N))])
...
def objective_function(pars, *args):
    N, I, P, beta, c_par, num_t_pars, num_k_pars, num_h_pars = args
    t_pars = pars[0:num_t_pars]
    k_pars = pars[num_t_pars:num_t_pars+num_k_pars]
    h_pars = pars[num_t_pars+num_k_pars:num_t_pars+num_k_pars+num_h_pars]
    if c_par:
        c = pars[-1]
    else:
        c = None
...
def minimize_objective(pars, N, I, P, beta, c_par, bounds, num_t_pars, num_k_pars, num_h_pars):
    return opt.minimize(objective_function, pars, args=(N, I, P, beta, c_par, num_t_pars, num_k_pars, num_h_pars), method="Nelder-Mead", bounds=bounds)

def optimize_model(N, I, P, beta, initial_pars, h_bounds=(1,4), t_bounds=(0,1), k_bounds=(10**-3,10**3), num_threads=60, num_t_pars=5, num_k_pars=4, num_h_pars=2, c=False):
    start = time.time()    
    min_k_order = np.log10(k_bounds[0])
    max_k_order = np.log10(k_bounds[1])
    min_c_order = np.log10(10**-3)
    ...
    # Define bounds
    bnds = [(min_t, max_t) for i in range(num_t_pars)] + [(10**min_k_order, 10**max_k_order) for i in range(num_k_pars)] + [(min_h, max_h) for i in range(num_h_pars)]
    if c:
        bnds.append((10**min_c_order, 10**max_c_order))
    bnds = tuple(bnds)

    # print("Length of bounds: %d" % len(bnds), flush=True)
    # print("Shapre of pars: %s" % str(initial_pars.shape), flush=True)

    # raise ValueError("Stop")

    # Optimize
    with Pool(num_threads) as p:
        results = p.starmap(minimize_objective, [(pars, N, I, P, beta, c, bnds, num_t_pars, num_k_pars, num_h_pars) for pars in initial_pars])
    ...
    if c:
        with Pool(num_threads) as p:
            ...
            ifnb_predicted =  p.starmap(get_f, [(final_pars[i,0:num_t_pars], 
                                                    final_pars[i,num_t_pars:num_t_pars+num_k_pars], 
                                                    N[j], I[j], P[j], 
                                                    final_pars[i,-1], 
                                                    final_pars[i,num_t_pars+num_k_pars:-1],
                                                    False) for i in range(len(final_pars)) for j in range(len(N))])
    else:
        with Pool(num_threads) as p:
            ...
            ifnb_predicted =  p.starmap(get_f, [(final_pars[i,0:num_t_pars],
                                                    ...
                                                    N[j], I[j], P[j],
                                                    None,
                                                    final_pars[i,num_t_pars+num_k_pars:],
                                                    False) for i in range(len(final_pars)) for j in range(len(N))])
...
def main():
    ...
    # Directories
    figures_dir = "parameter_scan_h/figures/"
    results_dir = "parameter_scan_h/results/"
    ...
    if c_par:
        ...
        result_par_names = ["t_1","t_3","t_4","t_5", "k1", "k2", "kn", "kp", "h1", "h2", "c"]
    else:
        # result_par_names = [r"$t_I$",r"$t_N$",r"$t_{I\cdotIg}$", r"$t_{I\cdotN}$", "k1", "k2", "kn"]
        result_par_names = ["t_1","t_3","t_4","t_5", "k1", "k2", "kn", "kp", "h1", "h2"]
    ...
    if args.param_scan:
        ...
        final_pars, ifnb_predicted, rmsd = calculate_grid(training_data, seed=0, num_threads=num_threads, num_t_pars=num_t_pars, num_k_pars=num_k_pars, num_h_pars=num_h_pars, c_par=c_par)
    ...
    if args.optimize:
        ...
        final_pars, ifnb_predicted, rmsd = optimize_model(N, I, P, beta, initial_pars, h_bounds=(1,4), t_bounds=(0,1), k_bounds=(10**-3,10**3), num_threads=num_threads, num_t_pars=num_t_pars, num_k_pars=num_k_pars, num_h_pars=num_h_pars, c=c_par)
    ...
    # Plot state probabilities
    if args.state_probabilities:
        raise NotImplementedError("State probabilities not implemented for this model.")
