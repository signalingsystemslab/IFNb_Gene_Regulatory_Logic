...
def optimize_model(N, I, beta, model_name, params, num_threads=40):
    print("###############################################\n")
    print("Optimizing model %s globally" % model_name)
    start = time.time()
    print("Starting random optimization at %s" % time.ctime())

    # rmsd = np.zeros(par_length)
    with Pool(num_threads) as p:
            res = p.starmap(three_site_objective, [(params[j], N, I, beta, model_name, "rmsd") for j in range(params.shape[0])])
            # test
            # res = p.starmap(test_objective, [(params[j], N, I, beta, model_name, "rmsd") for j in range(params.shape[0])])
            # print(res)
            rmsd = np.array(res)
...
def main():
    ...
    for model in model_par_numbers.keys():
        ...
        # print("Random parameter size: %d x %d" % (params.shape[0], params.shape[1]))
    ...
    results.to_csv("%s/three_site_random_results.csv" % results_dir)
    results_local.to_csv("%s/three_site_random_local_results.csv" % results_dir)
    ...
    plt.title("Best fit parameters with random opt for 3-site model")
    plt.savefig(os.path.join(figures_dir, "three_site_random_best_fit_params.png"), bbox_inches="tight")
