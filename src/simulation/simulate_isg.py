...
def main():
    ...
    parent_dir = "results/isg/"
    os.makedirs(parent_dir, exist_ok=True)
    num_states = 7

    print("\n##########\nSimulating IFNb, IFNAR, ISGF3, and ISGs\n##########\n", flush=True)

    for i in range(1):
        isg_half_life = pars["t_half_isg"]
        dir = "%s/isg_half_life_%d" % (parent_dir, isg_half_life)
        os.makedirs(dir, exist_ok=True)
        print("Saving results to %s" % dir, flush=True)
        ...
        with Pool(40) as p:
            ...
            lps_results = p.starmap(full_simulation, [(states0, t_params[i,:], pars, "LPS_%d" % i, "LPS", genotype, dir, num_states,
                                                         stim_time, None, False) for i in range(num_par_reps)])
            pic_results = p.starmap(full_simulation, [(states0, t_params[i,:], pars, "pIC_%d" % i, "pIC", genotype, dir, num_states,
                                                            stim_time, None, False) for i in range(num_par_reps)])
        ...
        pic_timecourses = [pic_results[i][0].y[:,:] for i in range(num_par_reps)]
        pic_timecourses = np.array(pic_timecourses)
        np.save('%s/pic_results.npy' % dir, pic_timecourses)
        ...
        pic_timecourses = np.load("%s/pic_results.npy" % dir)
        ...
        for state in range(num_states):
            ...
            # ax = fig.add_subplot(111)
            # for i in range(num_par_reps):
            #     ax.plot(t_eval, pic_timecourses[i,state,:], color=colors[state], alpha=0.1)
            # ax.set_xlabel("Time (min)")
            # ax.set_ylabel(r"%s (nM)" % state_names[state])
            # ax.set_title("poly I:C stimulation, all parameter sets")
            # ax.set_ylim(ylim)
            # plt.savefig("%s/pic_timecourses_%s.png" % (dir, state_titles[state]))
            ylim = plot_state_mult_pars(num_par_reps, t_eval, lps_timecourses, state, state_names, state_titles, colors, dir, "LPS")
            plot_state_mult_pars(num_par_reps, t_eval, cpg_timecourses, state, state_names, state_titles, colors, dir, "CpG", ylim)
            plot_state_mult_pars(num_par_reps, t_eval, pic_timecourses, state, state_names, state_titles, colors, dir, "pIC", ylim)

        pars = {**ifnar_pars, **other_pars}

    genotype = "p50KO"
    print("\n##########\nSimulating IFNb, IFNAR, ISGF3, and ISGs in p50 KO\n##########\n", flush=True)
