...
resid_df = read_csv("../data/ModelB1_res.csv",col_names = c("pt1","pt2","pt3","pt4","pt5","pt6","pt7","dset")) %>%
...
all(abs(x$rmsd-rmsd_df$rmsd)<0.00001)
...
p=ggplot(resid_df,aes(x=data_point,y=residual)) +
  ...
  ggtitle(expression("Model "*beta*"1 residuals from best fit model for each data set"))

ggsave(p, filename = str_c("../3-site-model/figs/B1_all_residuals.png"),
       height = 6, width = 9)
