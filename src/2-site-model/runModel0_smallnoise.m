...
irf_dev = max(exp_matrix.irf)/100;
irf_var = irf_dev^2;
nfkb_dev = max(exp_matrix.nfkb)/100;
nfkb_var = nfkb_dev^2;
...
for j = 1:ntvec
   parfor i = 1:numbPoints
    ...
    disp(j)
    save(['../data/model0_2statesmallnoise_b',num2str(j),'.mat'],...
...
for jth = 1:nfiles
    load(['../data/model0_2statesmallnoise_b',num2str(jth),'.mat']);
...
%
save('../data/model0_rmsd_2statesmallnoise.mat','minRmsd','minRmsdind','rmsdQuantile', 'minRmsdParam')

tnames = ["t3", "t2", "t1", "t4"];
M = [tnames; minRmsd; minRmsdParam];
save('../data/model0_minimums_2statesmallnoise.mat', 'M')
