...
def generate_synthetic_dataset(training_data, std_err, seed, number):
    ...
    while LPS_WT_val < LPS_IRF_2KO_val or PIC_IRF_2KO_val < PIC_IRF_3KO_val:
        ...
        # IFNb_synthetic = rng.normal(IFNb_vals, std_dev)
        # IFNb_synthetic = np.clip(IFNb_synthetic, 0, 1)
        # IFNb_synthetic = np.round(IFNb_synthetic, 3)
...
def generate_synthetic_data(training_data, std_err, num_datasets, original_seed):
    ...
    for i in range(num_datasets):
        seed += 1
        synthetic_data = pd.concat([synthetic_data, generate_synthetic_dataset(training_data, std_err, seed, i)])
    return synthetic_data
...
def main():
    ...
    for e in errors_tested:
        ...
        #     print("%s IRF range: %.2f, NFkB range: %.2f" % (name, group["IRF"].max()-group["IRF"].min(), group["NFkB"].max()-group["NFkB"].min()))
        ...
        pal = sns.cubehelix_palette(as_cmap=True, start=.6, rot=-.4, dark=0.2, light=0.95, hue=0.6)
